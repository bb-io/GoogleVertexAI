...
{
    public static object BuildBatchRequestObject(
        string userPrompt, 
        string systemPrompt, 
        PromptRequest pr, 
        string modelPath, 
        object? schema = null)
    {
        ...
        if (pr.Temperature.HasValue) 
            genCfg["temperature"] = pr.Temperature.Value;
        
        if (pr.TopP.HasValue) 
            genCfg["topP"] = pr.TopP.Value;
        
        if (pr.TopK.HasValue) 
            genCfg["topK"] = pr.TopK.Value;
